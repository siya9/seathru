...
def load_image_and_depth_map(img_fname, depths_fname, size_limit=1024):
    # Load the depth map using cv2
    depths = cv2.imread(depths_fname)#, cv2.IMREAD_GRAYSCALE)

    # Load the image using cv2 and normalize pixel values
    img = cv2.imread(img_fname)
    img = cv2.cvtColor(img, cv2.COLOR_BGR2RGB)
    img = cv2.normalize(img, None, 0, 1, cv2.NORM_MINMAX, cv2.CV_32F)

    # Resize both the image and depth map if needed
    height, width = img.shape[:2]
    print("The width of the image is", width)
    if height > size_limit or width > size_limit:
        scale_factor = size_limit / max(height, width)
        img = cv2.resize(img, (int(width * scale_factor), int(height * scale_factor)))
        depths = cv2.resize(depths, (int(width * scale_factor), int(height * scale_factor)))
    return img, depths
